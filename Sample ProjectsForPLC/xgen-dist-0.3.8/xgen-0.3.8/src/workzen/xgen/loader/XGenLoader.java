...
/**
 * Load the XGen xml schema into the package model. 
 * 
 * @author <a href="mailto:bmatlack@workzen.us">Brad Matlack</a>
 */
public class XGenLoader implements ILoader {

	private Logger logger = Logger.getLogger(XGenLoader.class);
	 ...
	 * @throws IOException
	 */
	public Document loadDocument(URL url, boolean validate)
		throws IOException {
	...
	private Field loadField(Element el) throws XGenException {
		...
		if (typename != null) {
			Type type = TypeMap.getType(typename);
			field.setType(type);
		}
